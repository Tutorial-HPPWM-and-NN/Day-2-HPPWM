...
def load_weights_from_npz(weights_path: Path):
    ...
    weight_keys = sorted(data.files, key=lambda name: int(name.split("_")[1]))
    return [data[key] for key in weight_keys]
...
def representative_dataset_generator(X_scaled: np.ndarray, num_samples: int = REPRESENTATIVE_SAMPLES):
    ...
    """
    n = min(num_samples, len(X_scaled))
    for i in range(n):
...
def convert_to_tflite_int8(model: tf.keras.Model, X_scaled: np.ndarray, output_path: Path):
    ...
    converter.representative_dataset = lambda: representative_dataset_generator(X_scaled)
    ...
    print(f"TFLite model exists: {output_path.exists()}")
    print(f"TFLite model size: {output_path.stat().st_size} bytes")


def quantize_input(x_float: np.ndarray, input_details: dict):
    scale, zero_point = input_details["quantization"]
    if scale == 0:
        return x_float.astype(input_details["dtype"])

    x_q = np.round(x_float / scale + zero_point)
    dtype = input_details["dtype"]
    qmin = np.iinfo(dtype).min
    qmax = np.iinfo(dtype).max
    x_q = np.clip(x_q, qmin, qmax).astype(dtype)
    return x_q


def dequantize_output(y_q: np.ndarray, output_details: dict):
...
def run_tflite_inference(tflite_model_path: Path, X_scaled: np.ndarray):
    ...
    for i in range(len(X_scaled)):
        ...
        if input_details["dtype"] == np.int8:
            x_input = quantize_input(x_sample, input_details)
        else:
            x_input = x_sample.astype(input_details["dtype"])

        interpreter.set_tensor(input_details["index"], x_input)
        interpreter.invoke()
        y_out = interpreter.get_tensor(output_details["index"])

        if output_details["dtype"] == np.int8:
            y_out = dequantize_output(y_out, output_details)
        else:
